...
def get_account_manager(
    ...
    return {
        "active_account": manager.active_account,
...
def _get_switch_safety(request: Request) -> AccountSwitchSafetyV2:
    ...
    if not isinstance(safety, AccountSwitchSafetyV2):
        raise HTTPException(
            status_code=503,
            detail="No se puede verificar la identidad del runtime.",
        )
    return safety
...
def get_switch_context(request: Request):
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="La identidad del runtime requiere revisión.",
        ) from None
...
def switch_account(
    payload: AccountSwitchRequestV2,
    request: Request,
):
    ...
    try:
        ...
        if str(exc) == "account_transition_failed_closed":
            return JSONResponse(
                status_code=503,
                content={
                    "status": "ACCOUNT_TRANSITION_UNCERTAIN",
                    "changed": None,
                    "reason": str(exc),
                    "detail": (
                        "Runtime bloqueado; reinicia y recupera "
