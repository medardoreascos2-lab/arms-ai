...
def target(h, name):
    matches = [
        key
        for key, row in h.c._catalog["accounts"].items()
...
def trade(h, close_price=None):
    ...
    result = r.trade_lifecycle_service.submit_signal(
        ...
        order_type="MARKET",
        risk_context={
            **target(h, h.c.identity.profile_name),
    ...
    if close_price is not None:
        r.trade_lifecycle_service.update_position(
...
def test_scenarios_a_to_n(hosted, scenario, url):
    ...
    a_events = (
        a.trade_lifecycle_service.execution_risk_gate_v1.logger.get_events()
    )

    if scenario in {"C", "G"}:
        a.account_state_manager_v2.record_daily_pnl(daily_pnl=-3000.0)

    if scenario == "D":
        trade(h, 7500.0)
        a.account_state_manager_v2.record_daily_pnl(daily_pnl=0.0)
    ...
    if scenario == "H":
        disk = (
            a.execution_state_store._durability.path.read_bytes()
        )
        for _ in range(3):
    ...
    assert b is not a
    assert (
        b.settings.account_balance
        == b.portfolio_manager_v2.get_available_balance()
        == 50000
    )
    assert b.settings.risk_percent == 0.25
    ...
    assert (
        b.execution_state_store._durability.path.parent.name
        == target(h, "B")["account_id"]
    )
    assert (
        h.client.get(
    ...
    assert (
        h.app.state.trade_lifecycle_service_v2
        is b.trade_lifecycle_service
    )
    assert a.execution_state_store._durability.retired

    with pytest.raises(AccountAdmissionRejected):
        a.execution_manager.prepare_order(
            signal=signal(),
            order_type="MARKET",
        )
    ...
    if scenario == "A":
        ...
        denied = a.trade_lifecycle_service.submit_signal(
            signal=signal(),
            order_type="MARKET",
        )
        ...
        with pytest.raises(AccountAdmissionRejected):
            a.trade_lifecycle_service.broker_connector_v2.submit_order(
                prepared_order={}
            )

        with pytest.raises(AccountAdmissionRejected):
            a.execution_state_store.save_to_file(
                file_path=a.execution_state_store.account_namespace
            )

        assert (
            a.execution_state_store.account_namespace.read_bytes()
            == saved
        )
    ...
    if scenario == "L":
        ...
        assert (
            b.trade_lifecycle_service.broker_connector_v2.account_id
            == target(h, "B")["account_id"]
        )

    if scenario == "G":
        assert (
            b.trade_lifecycle_service.execution_risk_gate_v1.logger.get_events()
            == []
        )
        trade(h, 10020.0)
        b.account_state_manager_v2.record_daily_pnl(daily_pnl=-123.0)
        b_events = (
            b.trade_lifecycle_service.execution_risk_gate_v1.logger.get_events()
        )
        b_before = snapshot(h)
        ...
        assert snapshot(h) == before
        assert (
            h.c.published.runtime.trade_lifecycle_service
            .execution_risk_gate_v1.logger.get_events()
            == a_events
        )
    ...
    if scenario in "MN":
        ...
        try:
            ...
            current = (
                restarted.published.runtime.execution_state_store.capture_state()
            )
            current.pop("captured_at")
            current.pop("account_identity")
...
@pytest.mark.parametrize(
    "corruption",
    ["other_account", "missing", "profile", "generation"],
)
def test_o_recovery_rejects_wrong_or_missing_identity(hosted, corruption):
    ...
    with pytest.raises(ValueError, match="namespace"):
        store.load_from_file(
            file_path=h.c.snapshot_path(
                target(h, "A")["account_id"]
            )
        )


def test_p_identical_profiles_with_distinct_ids_are_isolated(
    legacy_runtime,
    tmp_path,
):
        ...
        json.dumps(
            {
                "version": 2,
                "active_account": "A",
                "active_account_id": "PAPER-ONE",
                "runtime_generation": 1,
                "namespace_root": str(root),
                "accounts": {
                    "PAPER-ONE": {"profile_name": "A"},
                    "PAPER-TWO": {"profile_name": "A"},
                },
            }
        )
    )
    c = AccountRuntimeCoordinatorV2(
        config_path=config,
        namespace_root=root,
        registry=registry,
    )
    ...
    try:
        c.start()
        c.published.runtime.account_state_manager_v2.record_daily_pnl(
            daily_pnl=-3000.0
        )
        foreign = c.published.runtime.execution_state_store.capture_state()
        assert c.switch(
            account_id="PAPER-TWO",
            profile_name="A",
        )["changed"]

        with pytest.raises(ValueError, match="Cross-account"):
            c.published.runtime.execution_state_store.validate_state(
                state=foreign
            )

        assert not c.published.runtime.account_state_manager_v2.get_state()[
            "trading_blocked"
        ]
        assert c.switch(
        ...
        assert c.published.runtime.account_state_manager_v2.get_state()[
            "trading_blocked"
        ]
    finally:
        c.close()
...
def test_q_r_crash_boundary_recovers_only_committed_account(
    hosted,
    monkeypatch,
    phase,
):
    ...
    assert not [
        e
        for e in h.app.state.dashboard_event_bus_v2.get_event_history()
    ...
    try:
        c.start()
        assert c.identity.profile_name == (
            "B" if phase == "AFTER_PUBLISH" else "A"
        )

        if phase != "AFTER_PUBLISH":
            value = (
                c.published.runtime.execution_state_store.capture_state()
            )
            value.pop("captured_at")
            value.pop("account_identity")
            assert value == before
...
def test_s_t_switch_excludes_signals_execution_persistence_and_other_switch(
    hosted,
    monkeypatch,
):
    h = hosted
    ...
    try:
        ...
        blocked = source.trade_lifecycle_service.submit_signal(
            signal=signal(),
            order_type="MARKET",
        )
    ...
    assert h.c.identity.profile_name == "B"
...
def test_preexisting_operation_blocks_switch(hosted):
    ...
    with h.c.published.runtime.execution_state_store._durability.mutation():
        with pytest.raises(
            ...
            match="durable_operation_pending",
        ):
            h.c.switch(**target(h, "B"))
...
def test_new_b_rejects_stale_a_signal_before_prepare(
    hosted,
    monkeypatch,
):
    h = hosted
    ...
    for changes in (
        ...
        value = r.trade_lifecycle_service.submit_signal(
            signal=signal(),
            order_type="MARKET",
            risk_context=risk,
        )
        assert value["accepted"] is False
        assert value["prepared_order"] is None

    assert (
        r.trade_lifecycle_service.broker_connector_v2.get_fills()
        == []
    )


def test_legacy_global_snapshot_is_never_silently_adopted(
    legacy_runtime,
    tmp_path,
):
...
def test_real_process_exit_during_publication(hosted, phase):
    ...
    profiles = {
        key: asdict(row)
        for key, row in h.c.registry.accounts.items()
    }
...
registry.accounts = {
    name: FundingFirmProfile(**row)
    for name, row in profiles.items()
}
c = module.AccountRuntimeCoordinatorV2(
    config_path=config,
    namespace_root=root,
    registry=registry,
)
...
"""
    ...
    if phase.startswith("SELECTOR_TEMP"):
        assert (
            json.loads(h.config.read_text())["active_account"]
            == "A"
        )
        with pytest.raises(
            AccountSwitchRejected,
    ...
    try:
        restarted.start()
        assert restarted.identity.profile_name == (
            "B" if committed_b else "A"
        )
        state = (
            restarted.published.runtime.execution_state_store.capture_state()
        )
        state.pop("captured_at")
        state.pop("account_identity")
...
def test_failure_before_commit_preserves_a_and_allows_valid_a_signal(
    hosted,
    monkeypatch,
    phase,
):
    ...
    assert not h.c.switching
    assert h.c.phase == "READY"
    assert trade(h, 10010.0)["execution_risk_gate"]["account"] == "A"


def test_error_after_commit_is_uncertain_and_never_reports_unchanged(
    hosted,
    monkeypatch,
):
    h = hosted
...
def test_signal_via_http_uses_b_risk_sizing_and_paper_identity(hosted):
    ...
    assert value["risk_evaluation"]["risk_amount"] == 125.0
    assert value["risk_evaluation"]["maximum_daily_loss"] == 1000.0
    assert value["risk_evaluation"]["maximum_total_drawdown"] == 2000.0
    assert (
        h.c.published.runtime.trade_lifecycle_service
        .broker_connector_v2.account_id
        == target(h, "B")["account_id"]
    )
...
def test_asgi_default_builds_coordinated_accounts(
    legacy_runtime,
    tmp_path,
    monkeypatch,
):
    ...
    with TestClient(app, headers=ADMIN_HEADERS) as client:
        ...
        assert (
            app.state.runtime_state_path_v2.parent.name
...
@pytest.mark.parametrize(
    "source",
    ["pending", "protection", "oco", "order", "portfolio", "unknown"],
)
def test_coordinated_switch_retains_010a_activity_blockers(
    hosted,
    monkeypatch,
    source,
):
    ...
    if source == "pending":
        r.execution_state_store._durability.operation = {
            "operation_id": "pending"
        }
    elif source == "protection":
        monkeypatch.setattr(
            r.protective_order_registry,
            "list_protections",
            lambda **kw: [{}],
        )
    elif source == "oco":
        ...
        monkeypatch.setattr(
            r.trade_lifecycle_service.broker_connector_v2,
            "get_orders",
            lambda: [{"status": "PENDING"}],
        )
    elif source == "portfolio":
        monkeypatch.setattr(
            r.portfolio_manager_v2,
            "get_open_positions",
            lambda: [{}],
        )
    else:
        monkeypatch.setattr(
            r.trade_lifecycle_service,
    ...
    assert (
        r.execution_state_store._durability.path.read_bytes()
        == disk
    )
...
def test_target_snapshot_failure_never_publishes_or_inherits_source(
    hosted,
    damage,
):
    h = hosted
    ...
    trade(h, 10010.0)
    path = (
        h.c.published.runtime
        .execution_state_store._durability.path
    )
    assert switch(h, "A").status_code == 200
    ...
    if damage == "corrupt":
        ...
        if damage == "missing_identity":
            ...
            saved["account_identity"]["account_id"] = target(
                h,
                "A",
            )["account_id"]
...
def test_selector_io_failure_is_safe(hosted, monkeypatch, when):
    ...
    assert response.status_code == (
        503 if when == "after_replace" else 409
    )
...
def test_switch_rejects_while_http_consumer_uses_source(hosted):
    ...
    thread = Thread(
        target=lambda: results.append(
            h.client.get("/test-account-read")
        )
    )
    thread.start()
...
def test_restored_b_pnl_is_used_for_new_sizing(hosted):
    ...
    trade(h, 10010.0)
    capital = (
        h.c.published.runtime.portfolio_manager_v2
        .get_available_balance()
    )
    assert capital == 50020.0
    assert switch(h, "A").status_code == 200
    assert switch(h, "B").status_code == 200
    result = trade(h)
    assert (
        result["risk_evaluation"]["risk_amount"]
        == capital * 0.25 / 100
    )
...
def test_final_review_published_target_remains_frozen_until_ready(
    hosted,
    monkeypatch,
):
    h = hosted
    ...
    def probe(phase):
        ...
        if phase == "AFTER_PUBLISH":
            ...
            with pytest.raises(AccountAdmissionRejected):
                r.execution_manager.prepare_order(
                    signal=signal(),
                    order_type="MARKET",
                )

            with pytest.raises(AccountAdmissionRejected):
                r.paper_execution_engine.execute(
                    prepared_order={}
                )

            with pytest.raises(AccountAdmissionRejected):
                r.trade_lifecycle_service.broker_connector_v2.submit_order(
                    prepared_order={}
                )
...
def test_final_review_coordinated_recovery_never_synthesizes_legacy_records(
    hosted,
):
    h = hosted
    store = h.c.published.runtime.execution_state_store
...
def test_final_review_source_checkpoint_failure_keeps_all_admission_closed(
    hosted,
    monkeypatch,
):
