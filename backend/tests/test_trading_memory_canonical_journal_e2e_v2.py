...
def test_trading_memory_reads_canonical_closed_trade():
    app = create_app()
    ...
    assert canonical_trade.symbol == "NQ"
    assert canonical_trade.direction == "LONG"
    assert canonical_trade.status == "CLOSED"
    ...
    assert payload["dominant_strategy"] == "UNKNOWN"
    assert payload["memory_quality"] == "REVIEW"

    assert payload["winning_patterns"] == []

    assert payload["losing_patterns"] == [
        "BUY - UNKNOWN"
    ]
