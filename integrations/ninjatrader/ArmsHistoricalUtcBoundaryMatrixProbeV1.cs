// R5.6 offline-authored diagnostic host. Installation/native use requires a later gate.
using System;
...
{
    public class ArmsHistoricalUtcBoundaryMatrixProbeV1 : Indicator
    {
        private readonly object sync = new object();
        // This gate covers only cancellation admission / the final rename, never
        // matrix, hashing, writes, flushes or the ordinary state lock.
        private readonly object sealPublication = new object();
        private int terminationIntent;
        private ArmsHistoricalUtcBoundaryMatrixProbeV1 owner;
        ...
        private int requestAttempts, constructorAttempts, matrixAttempts;
        ...
        private BoundaryMatrixResult matrix;
        private const int MaximumEvidenceBytes = 262144;
        private const string EvidenceName = "historical-utc-boundary-matrix.json";
        private const string SealName = "historical-utc-boundary-matrix.done.json";
        ...
        protected override void OnStateChange()
        {
            ...
            lock (sync)
            {
                ...
                {
                    Name = "ArmsHistoricalUtcBoundaryMatrixProbeV1";
        ...
        private Dictionary<string, object> RequestFact()
        {
            ...
            return Map("instrument", request.Instrument.FullName, "master", request.Instrument.MasterInstrument.Name,
                ...
                "submitted_from", BoundaryTime.Of(from), "submitted_through", BoundaryTime.Of(through),
                "actual_from", BoundaryTime.Of(request.FromLocal), "actual_through", BoundaryTime.Of(request.ToLocal),
        ...
        private Dictionary<string, object> Snapshot()
        {
            ...
            return Map("identity", "RETURNED_BARS_1", "callback_request_identity_verified", true, "count", count,
                "first", BoundaryTime.Of(first), "last", BoundaryTime.Of(last),
        ...
        private void Completed(BarsRequest received, ErrorCode error, string ignoredProviderMessage)
        {
            lock (sync)
            {
                ...
                try
                {
                    ...
                    Need(matrixAttempts == 0); matrixAttempts++;
                    traversing = true;
                    try { matrix = HistoricalUtcBoundaryMatrixDiagnosticV1.Run(returnedBars, initial, limit, CheckContext); }
        ...
        private void RequireCompleteConditionalRow()
        {
            Need(matrix != null);
            // Preserve the reviewed A-E abort contract, before E decides whether F is needed.
            if (!matrix.conditional_required.HasValue) return;
            int expected = matrix.conditional_required.Value ? 12 : 10;
            Need(matrix.outcome == "COMPLETE" && matrix.interpretation_allowed
                && matrix.iterator_constructor_attempts == expected
                && matrix.getnextsession_attempts == expected / 2 * 3
                && matrix.observations.Count == expected);
            for (int i = 0; i < expected; i++)
            {
                BoundaryObservation item = matrix.observations[i];
                Need(item != null && item.case_id == HistoricalUtcBoundaryMatrixDiagnosticV1.Cases[i / 2]
                    && item.mode == (i % 2 == 0 ? "PRIMED_REUSED" : "FRESH_DIRECT")
                    && item.constructor_ordinal == i + 1 && item.phase == "COMPLETE"
                    && item.observation != null);
                // A bounded case exception is an observation. Constructor/prime-only entries are not.
            }
        }
        private void Publish()
        {
            ...
            EnvironmentContract();
            var evidence = Envelope("arms.r56.historical-utc-boundary-matrix.v1");
            ...
            evidence.Add("matrix", matrix);
            evidence.Add("initial_query", BoundaryTime.Of(DateTime.SpecifyKind(from.AddDays(-2), DateTimeKind.Utc)));
            evidence.Add("calendar_through", BoundaryTime.Of(DateTime.SpecifyKind(through.AddDays(8), DateTimeKind.Utc)));
            evidence.Add("request_attempts", requestAttempts); evidence.Add("request_constructor_attempts", constructorAttempts);
            evidence.Add("matrix_attempts", matrixAttempts);
            byte[] raw = Encoding.UTF8.GetBytes(Json(evidence) + "\n"); Need(raw.Length <= MaximumEvidenceBytes);
            var seal = Envelope("arms.r56.historical-utc-boundary-matrix.seal.v1");
            ...
            EnvironmentContract();
            RequireCompleteConditionalRow(); // Retain partial body, but never create its completion seal.
            ...
            lock (sealPublication)
            {
                ...
                RequireCompleteConditionalRow();
                File.Move(Path.Combine(folder, SealName + ".tmp"), Path.Combine(folder, SealName));
            }
            terminal = true;
            try { Print("ARMS_R56_DIAGNOSTIC_COMPLETE_CONTRACT_ONLY"); } catch { }
        }
        private void Fail(Exception error)
        {
            terminal = true;
            try { Print("ARMS_R56_DIAGNOSTIC_FAILED_" + HistoricalUtcBoundaryMatrixDiagnosticV1.Error(error)); } catch { }
            Close();
        }
        private void Close()
        {
