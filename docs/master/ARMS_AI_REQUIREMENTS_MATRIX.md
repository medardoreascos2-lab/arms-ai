...
| SAFE-003 | Risk | Missing, stale, inconsistent, invalid, or unauthorized risk data fails closed | Policy, risk architecture, focused tests, risk characterization | PARTIALLY_IMPLEMENTED | CRITICAL | Characterized paths fail closed; complete cross-path coverage remains open |
| SAFE-004 | Execution | PAPER execution cannot select or reach a LIVE or real-money broker | PAPER components, account isolation, policy, compatibility characterization | PARTIALLY_IMPLEMENTED | CRITICAL | LIVE remains blocked; complete mechanical isolation is not proven |
